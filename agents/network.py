...
def build_net(minimap, screen, info, msize, ssize, num_action, ntype):
    if ntype == 'atari':
        ...
        return build_custom(minimap, screen, info, msize, ssize, num_action)
    else:
        raise 'FLAGS.net must be atari, fcn, or custom'
...
def build_custom(minimap, screen, info, msize, ssize, num_action):
    # Extract features
    mconv1 = layers.conv2d(tf.transpose(minimap, [0, 2, 3, 1]),
                           num_outputs=16,
                           kernel_size=5,
    ...
    spatial_action_pre = layers.conv2d(feat_conv,
                                   num_outputs=1,
                                   kernel_size=1,
                                   stride=1,
                                   activation_fn=None,
                                   scope='spatial_action_pre')
    feat_spatial_action = tf.concat([layers.flatten(spatial_action_pre), non_spatial_action], axis=1)
    spatial_action = layers.fully_connected(feat_spatial_action,
                                                num_outputs=ssize ** 2,
                                                 activation_fn=tf.nn.softmax,
                                                 scope='spatial_action')
