...
class A3CAgent(object):
    ...
    def set_action_spec(self, action_spec):
    ...
    def random_choice(self, obs):
        valid_actions = obs.observation['available_actions']
        action_id = np.random.choice(valid_actions)
        args = [[np.random.randint(size) for size in arg.sizes] for arg in self.action_spec[0].functions[action_id].args]
    ...
    def update(self, rbs, disc, lr, cter):
        ...
        for i, [obs, action, next_obs] in enumerate(rbs):
            ...
            #reward = obs.reward
            reward = 0.25 * (next_obs.observation['score_cumulative'][0] - obs.observation['score_cumulative'][0])
